    ...
    <div className="hub-container" style={{ position: 'relative', overflow: 'hidden', paddingBottom: '50px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <h2 style={{ fontSize: '2rem', fontWeight: 800, margin: 0 }}>
          <span style={{ color: 'var(--primary)' }}>Live</span> Management Hub
        </h2>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '40px', flexWrap: 'wrap', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: '15px' }}>
              <Link href={`/dashboard/scoring?eventId=${currentEvent.id}`} className="btn btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ✍️ Manage Results
              </Link>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }} className="mobile-grid-1">
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div className="glass-panel" style={{ padding: '20px', textAlign: 'center' }}>
                  <div style={{ fontSize: '2rem', fontWeight: 900, color: 'var(--success)' }}>
                    {currentEvent.stats.publishedPrograms}
                  </div>
              ...
              <div style={{ marginTop: '20px', padding: '20px', background: 'rgba(255,255,255,0.02)', borderRadius: 'var(--radius-lg)', border: '1px solid rgba(255,255,255,0.05)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', fontSize: '0.9rem' }}>
                  ...
                  <span>{Math.round((currentEvent.stats.publishedPrograms / currentEvent.stats.totalPrograms) * 100)}%</span>
                </div>
                <div style={{ height: '10px', background: 'rgba(255,255,255,0.05)', borderRadius: '5px', overflow: 'hidden' }}>
                  <div style={{ 
                    height: '100%', 
                    width: `${(currentEvent.stats.publishedPrograms / currentEvent.stats.totalPrograms) * 100}%`,
            ...
            <div>
              ...
              <div className="glass-panel" style={{ maxHeight: '300px', overflowY: 'auto', padding: '10px' }}>
                {currentEvent.pendingList.length > 0 ? (
                  currentEvent.pendingList.map((prog: any) => (
                    <div key={prog.id} style={{ 
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{prog.category}</div>
                      </div>
                      <Link href={`/dashboard/scoring?programId=${prog.id}`} style={{ 
                        fontSize: '0.7rem', 
                        padding: '4px 10px', 
                  ...
                  <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>
                    All entries completed for this event! 🎉
                  </div>
                )}
              </div>
            </div>
